...
Four complete scripts, end to end. Each traces a whole arc — world read, topology, loop or fan-out, salvage, artifacts, return shape — so read one when you want to see how the pieces fit together rather than look up an isolated shape (`patterns.md` serves that purpose).

Each script is submitted as the `script` argument of `CreateWorkflow`, whose description carries the API surface all four are written against.

---

## 1. Adversarial implement ↔ verify loop

Two subagents with opposing jobs, looping until the verifier runs out of ways to break the implementation. The shape transfers to anything pairing an author with a skeptic: code and property test, proof and counter-example, schema and fuzzer.
...
**Why it is written this way.** Both subagents are created outside the loop, so each retains what it learned: the implementer remembers the approaches already tried, the verifier remembers which attacks already landed. Both genuinely edit and run files.

The two exchange **paths and descriptions, never file contents**; each reads the other's work through its own tools. Every round is reported as it happens, so a run dying on round six still shows five rounds of counter-examples. The return value carries `verified` and `rounds`, letting a caller tell convergence from exhaustion — and it opens with `conclusion` and closes with `notCovered`, because that value is what the main agent retells.

`verified` does not rest on the verifier's word. The verifier is a subagent, and "I ran the tests" from a subagent is a claim. The closing `world.run` executes the test file for real, and its exit code is what the result rests on. The implementer's persona names the exact commands it must run for the same reason: "run the tests" would let a subagent pick whatever turns green fastest.

Two artifacts, each aimed at a different moment. The metrics tile serves whoever is watching the loop — round and open counter-examples, fed by the `report` call the script was making anyway. The markdown report serves afterwards: the same facts as the return, in the long form the user keeps. The tests themselves go unpublished — they live in the repository, and a card for a file the user opens in the editor regardless says nothing new.

---

## 2. Flaky test triage

Scan first, then plan a fix under critique, then judge every finding independently. Three topologies in one script, each picked for its own reason.
...
**Why it is written this way.** The judges are fresh per finding while the planner/reviewer pair is hoisted — opposite choices, both deliberate. Judging needs independence, so a shared context would corrupt it; planning needs memory, so a fresh planner each round would waste it. The confirmers are a third kind: fresh, per finding, told not to edit, because a judge's verdict is an opinion about a description and only re-running the test makes it a fact. Whatever they cannot reproduce stays in the report as `unconfirmed`. Each confirmer trails its own judge inside one callback, so the fan-out has one join rather than two: confirming candidate three never waits on candidate forty's judge.

The two early returns matter. A scan finding nothing and a judging pass rejecting everything are both *successes* with an empty result rather than failures, and each avoids paying for a planning loop over an empty list. Both still return the full report shape, with a `conclusion` naming which of the two occurred. Findings are reported before planning begins, so even a planning loop that never converges still hands back the confirmed findings.

The board gives a watcher the fan-out as it happens: every candidate appears the moment the scan returns and changes column as a judge or confirmer rules, because a later item carrying the same `testId` replaces the card. The report is published once, after the planning loop, which is why the loop `break`s toward a single return rather than returning from inside. The two early returns publish nothing: "No flaky tests found" is a one-line answer, and a page restating it would be noise.

---

## 3. Call-site migration

Discover, transform in parallel, then verify. The transforming subagents write files, which makes this the one shape where isolation and verification genuinely matter.
...
**Why it is written this way.** `files.grep` narrows ahead of the fan-out: globbing `src/**/*.ts` and asking each file whether it uses the old API would spend a session per file to learn what one search already knew. Since `grep` rejects rather than truncating past its cap, the path list is either complete or absent — never quietly partial, which for a migration is the difference between finished and silently half-done.

Both subagents in the fan-out carry the path in their name, and that is not decoration. A run rejects two subagents sharing a name, so `agent("migrator")` inside the `map` would not survive its second file. The per-path names are also what a corrected re-run (`AmendWorkflow`) matches its imported cache against, so re-running after fixing one detail does not re-migrate the files that already came out clean.

The checker is a **separate subagent** from the migrator, and that separation is what makes the check mean anything: a subagent grading its own migration grades generously. It builds and tests, and the ask is what forbids patching whatever it finds broken. The checker runs inside the same callback as its migrator — a per-item pipeline with one join — so the first file is verified while the fortieth is still being rewritten, instead of every checker queuing behind a `Promise.all` for the slowest migration. The `try`/`catch` sits per file, so one unmigratable file costs one file rather than the run. The return value separates `migrated`, `failed` and `skipped`, because "we changed 40 files" is not an outcome anyone can act on.

The fan-out verifies each file; the `world.run` after it verifies the tree. Those are different checks. Forty files that each pass their own tests can still fail to compile together, and the caller in a file nobody migrated is precisely what a per-file checker never opens. The whole-suite run is what the user would run before merging, so it runs here, once, with the timeout a real suite needs — and when it fails, `conclusion` declares the result unverified rather than letting forty green rows speak for the tree.

The table shows the fan-out live: one row per file, keyed by path, filled as each checker answers, with skipped files landing in it too. The closing report is that same list in prose for the user to keep. The migrated files themselves go unpublished — they are the repository, and the user reads them there. The early return for "nothing to migrate" publishes nothing, because a one-line answer needs no page.

---

## 4. Prover loop gated by the real checker

One subagent doing open-ended repair, behind a gate that cannot be talked past: `world.run` executes the actual proof checker, and the loop advances on its exit code rather than on anyone's claim. Compare example 1, where the skeptic is another subagent — the right choice when breaking the work takes creativity. When a command can render the verdict, let the command render it.
...
**Why it is written this way.** The prover is created once and keeps its context, so round seven remembers what rounds one through six tried. The checker is not a subagent at all: `world.run` runs the real command, a nonzero exit returns as a **value**, and the loop's normal path reads `exitCode` and hands `stderr` to the prover as the next round's instructions — no `catch` anywhere, because rejections are reserved for the world failing to answer (spawn failure, timeout, over-cap output).

Two checks here, and they are not interchangeable. `lake env lean` on one file is the fast tier: it drives the rounds because it answers in seconds, and it reads stderr alongside the exit code because a proof still saying `sorry` exits 0 with only a warning. `lake build` across the whole project is the strong tier — the check the request is really about — and it runs once at the end whether or not the fast tier already passed. A loop gated on the fast tier alone would report "proved" for a file still containing `sorry`, and would never learn that the new proof breaks a module importing it. Which two commands fill these roles is a fact about the repository, read from its README or build files before the script is written, not a habit carried over from the last project.

The command name is a literal while the varying part — the target path — rides in the args array. That is the rule, and it is also what the user sees and approves at confirmation. The long `timeoutMs` is deliberate: proof checking is allowed to be slow, and the per-call override exists precisely for build-sized checks. Every round is reported as it lands, so a run dying on round eight still shows seven rounds of attempts. Before wiring a different checker command, check what its stderr actually looks like with a one-line `EvalWorkflowSnippet` call — a parse written against a guess is the parse that breaks.

One artifact, deliberately. The report lists every attempt, which says more than `conclusion` can, so it earns its page. There is no dashboard: the only state a watcher could track is the round number and whether the checker passed, and the phase timeline lighting up "Repair what the checker rejected" again already conveys exactly that. A metrics tile would repeat it, and the proof file belongs to the repository to show rather than to a card.
